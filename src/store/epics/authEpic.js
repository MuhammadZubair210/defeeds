...
export default class AuthEpic {
  ...
  static signinEpic = (action$) =>
    action$.ofType(SIGNIN).switchMap(({ payload }) => {
      return HttpService.post(Path.LOGIN, payload).switchMap(({ response }) => {
  ...
  static signupEpic = (action$) =>
    action$.ofType(SIGNUP).switchMap(({ payload }) => {
      return HttpService.post(Path.SIGNUP, payload).switchMap(
        ({ response }) => {
          if (response.err) {
            return Observable.of({
              type: SIGNUP_FAILURE,
  ...
  static demoEpic = (action$) =>
    action$.ofType(DEMO).switchMap(() => {
      return HttpService.get(Path.GITHUB).switchMap(({ response }) => {
        console.log(response);
        if (response) {
          return Observable.of({
            type: DEMO_SUCCESS,
  ...
  static secondDemoEpic = (action$) =>
    action$.ofType(SECOND_DEMO).switchMap(({ payload }) => {
      return HttpService.post(Path.LOGIN, payload).switchMap(({ response }) => {
        console.log(response);
        if (response) {
          return Observable.of({
